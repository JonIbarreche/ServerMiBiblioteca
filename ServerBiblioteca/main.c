...
//		inicioAdmin(db, result);
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
		LOG_PRINT("Failed. Error Code : %d", WSAGetLastError());
		return -1;
	}

	LOG_PRINT("Initialised.\n");

	//SOCKET creation
	if ((conn_socket = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
		LOG_PRINT("Could not create socket : %d", WSAGetLastError());
		WSACleanup();
		return -1;
	}
	...
	if (bind(conn_socket, (struct sockaddr*) &server,
		...
		LOG_PRINT("Bind failed with error code: %d", WSAGetLastError());
	...
	if (listen(conn_socket, 1) == SOCKET_ERROR) {
		LOG_PRINT("Listen failed with error code: %d", WSAGetLastError());
		closesocket(conn_socket);
		WSACleanup();
		return -1;
	}
	...
	LOG_PRINT("Waiting for incoming connections...\n");
	int stsize = sizeof(struct sockaddr);
	comm_socket = accept(conn_socket, (struct sockaddr*) &client, &stsize);
	// Using comm_socket is able to send/receive data to/from connected client
	if (comm_socket == INVALID_SOCKET) {
		LOG_PRINT("accept failed with error code : %d", WSAGetLastError());
		closesocket(conn_socket);
		WSACleanup();
		return -1;
	}
	LOG_PRINT("Incomming connection from: %s (%d)\n", inet_ntoa(client.sin_addr),
	...
	LOG_PRINT("Waiting for incoming commands from client... \n");
	do {
		recv(comm_socket, recvBuff, sizeof(recvBuff), 0);

		LOG_PRINT("Command receivede: %s \n", recvBuff);
...
//					printf("adios4444");
	...
	if (result != SQLITE_OK) {
		LOG_PRINT("Error closing database\n");
		LOG_PRINT("%s\n", sqlite3_errmsg(db));
	}
